...
router.get(async (req, res) => {
  try {
    ...
    const [connectionsResult, versionResult, maxConnectionsResult] =
      await Promise.all([
        database.query(`
                SELECT COUNT(*) 
    ...
    res.status(200).json({
      ...
      version:
        ...
        "Não foi possível obter a versão do servidor",
    });
  } catch {
    res
      .status(500)
      .json({ error: "Erro ao verificar o status do banco de dados" });
  }
});
